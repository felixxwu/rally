...
import { minAirResistance, oldCarPosition } from '../../refs';
...
export function updateCar() {
  ...
  const inverseTravel = getDirectionOfTravel().multiplyScalar(-airResistance.current);
  ...
  if (keysDown.a) {
    carTorque.add(new THREE.Vector3(0, steerPower.current, 0));
  }
