...
export const Experience = ({setSpeed}: {setSpeed: (speed: number) => void}) => {
  ...
  const handleMovement = (tireForce: THREE.Vector3, airResistance: THREE.Vector3, angle: number, speed: number) => {
    if (!isOnFloor.current || !cube.current) {
      return;
    }
    const steerTorque = speed < 0.01 ? 0 : angle > 1 ? -steer : steer;
    if (rightPressed) {
  ...
  useFrame((_state, delta) => {
    ...
    const tireForce = spacePressed ? directionOfTravel.clone().multiplyScalar(-10).clampLength(0, maxTireForce) : sideTireForce

    const airResistance = directionOfTravel.clone().multiplyScalar(directionOfTravel.length() * -airResistanceScale)
